...
# CSV export URL format for public sheet
GOOGLE_SHEET_CSV_URL = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/export?format=csv"

@st.cache_data(ttl=60)  # Refresh every 60 seconds
def load_data():
    try:
        # Try loading directly from Google Sheet
        df = pd.read_csv(GOOGLE_SHEET_CSV_URL)
        return df
    except Exception as e:
        # Fallback structured dataset based on LS Blade Monitoring Records
        fallback_data = [
...
machine_filter = st.sidebar.multiselect(
    ...
    options=["LS-05", "LS-06", "LS-08"],
    default=["LS-05", "LS-06", "LS-08"]
)

status_filter = st.sidebar.multiselect(
    "เลือกสถานะ (Status):",
    options=["ปกติ (Safe)", "เฝ้าระวัง (Warning)", "วิกฤต (Critical)"],
    default=["ปกติ (Safe)", "เฝ้าระวัง (Warning)", "วิกฤต (Critical)"]
)
